...
var flag=false;
var SubDjs=document.querySelector(".Sub_djs");
var SubGuan=document.querySelector(".Sub_guan");
...
button.addEventListener("touchstart",function () {
    ...
    var numS=3;
    if (flag==true){
        Submitted.style.left=0+"px";
    }
    var tt=setInterval(function () {
        SubDjs.innerHTML=numS;
        numS--;
        console.log(numS)
        if (numS==-1){
            clearInterval(tt)
            Submitted.style.left=100+"%";
        }
    },1000);
    //关闭提示页
    SubGuan.addEventListener("touchstart",function () {
        Submitted.style.left=100+"%";
        clearInterval(tt)
    });
});
...
mandatoryIphone.addEventListener("touchstart",function () {
    Tziphone.style.background="blue"
});
//*的出现
